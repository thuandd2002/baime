...
class url_process {
    ...
    public function type_Info($type) {
        ...
        switch ($type) {
            ...
            case 'tracnghiem':
                $str['page'] = 'tracnghiem';
                $str['tableitem'] = 'tracnghiem';
                $str['id_item'] = 'id_tracnghiem';
                $str['adminpage'] = 'tracnghiem';
                $str['titlemenu'] = 'Trắc nghiệm';
                $str['data_type'] = "tracnghiem";
                break;				
	...
	function compile_uri($string) {
		
		// Bỏ dấu tiếng Việt 
		$string = RemoveSign($string);
		
		// Giữ lại chữ cái tiếng Anh, tiếng Hàn (Hangul), số và khoảng trắng
		$string = preg_replace('/[^a-zA-Z0-9\x{AC00}-\x{D7A3}\s]/u', '', $string);
		...
		// Loại bỏ dấu gạch ở đầu và cuối
		$string = trim($string, '-');
