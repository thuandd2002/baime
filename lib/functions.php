...
if (!function_exists('dd')) {
  function dd(...$vars)
  {
    echo '<pre style=" background:#111; color:#0f0; padding:15px; font-size:14px; line-height:1.5; overflow:auto; ">';
    foreach ($vars as $var) {
      var_dump($var);
    }
    echo '</pre>';
...
if (!function_exists('pr')) {
  function pr(...$vars)
  {
    echo '<pre style=" background:#222; color:#0ff; padding:15px; font-size:14px; line-height:1.5; overflow:auto; ">';
    foreach ($vars as $var) {
      var_dump($var);
    }
    echo '</pre>';
...
function sendmail($to,$subject,$contentmail, $name, $attachArr='', $reply = '')	{
	global $CONFIG;
	...
	$mail->Username = "noreply.bmgrmailer@gmail.com";
	$mail->SMTPAuth = true;
	$mail->Password = "wywnmihewydhisuc";
	$mail->From = "noreply.bmgrmailer@gmail.com"; // Reply to this email
	...
	$to_arr =  explode(",",$to); //Send to multi email by comma
	foreach($to_arr as $to_email){
		$mail->AddAddress(trim($to_email," "),$name);
	}
	...
	$mail->Body = $contentmail ; //HTML Body
...
function send_gmail($to,$subject,$contentmail, $name, $attachArr='', $reply = '')	{
	global $CONFIG;
	...
	if ($CONFIG['smtp_host'] == ""){
		...
		$mail->Host = "smtp.gmail.com";
		$mail->Username = "tb.bmgrmailer@gmail.com";
		$mail->Password = "mInh@!@#456";
		$mail->From = "tb.bmgrmailer@gmail.com"; // Reply to this email

	}else{
		$mail->Host = $CONFIG['smtp_host'];
		$mail->Username = $CONFIG['smtp_username'];// your SMTP username or your gmail username
		$mail->Password = $CONFIG['smtp_password']; // your SMTP password or your gmail password
		$mail->From = $CONFIG['smtp_username']; // Reply to this email
	}

	$mail->FromName= $CONFIG['site_name']; // Name to indicate where the email came from when the recepient received
...
function parseVideoUrl($url){
	...
	if (strpos($url,"=") !== false) {
		...
		$video = str_replace("https://youtu.be/",'',$url);
	}
...
/*single file upload*/
function uploadFileToSever($inputname = "uploaded_file"){
    ...
    if ($uploaded_size == 0) {
            $msg .= "Không xác định dung lượng (0) ! <br/>";
            $file_ok = 0;
    }
    ...
    if ($file_ok == 0) {
            ...
            if (move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $target)) {
                    ...
                    $file_ok = 1;
                    $msg = "Tải file lên server thành công. <br/>";
            } else {
...
/*
multi file upload
<input name="uploaded_files[]" type="file" multiple class="txt-contact "   placeholder="File attach" />
*/
function multiUploadFile(){
    global $cache_image_path ;
    $data = array();
	...
	$my_path = $_SERVER['DOCUMENT_ROOT'] . "/uploaded/files/";
	$image_text_link = "";
	foreach($img_desc as $val)
	{
		$filename = $val['name'];
		$filetype = $val['type'];
		$filesize = intval($val['size']); //byte

		if ($filetype == "image/jpeg" || $filetype == "image/png"){

			move_uploaded_file($val['tmp_name'],$my_path.$filename);
			//resize image
			$image_link = $cache_image_path . resizeimage1(800, 600,"/uploaded/user_files/" . $filename, false);
			$image_text_link = $image_text_link . "<a href='$image_link' target='_blank'>$image_link</a> <br/>";
			$file_ok = 1;
			$msg = "OK";
		}
	}
	$data['msg'] = $msg;
	$data['filePath'] = $image_text_link ;
...
//Message box use Modal box
function messageBox($messageContent,$messageType = "ok") {
    ...
    $messageBox = ' <div id="popupAlert" style="max-width:400px;" class="modal-box">
    <div class="modal-body">
        <div class="">
            <div class="pop-content">
                <div class="c20"></div>
                 <div style="text-align:center; color:#222; ">'. $icon . ' '.  $messageContent .' </div>
                <div class="c20"></div>
            </div>
        </div>
    </div>
