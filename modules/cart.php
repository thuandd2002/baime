...
if ($_REQUEST['code'] == 'save') {
	...
	$tpl->assignGlobal("csrf_token", $_SESSION['csrf_token'] );
...
function saveCart() {
    ...
    if (strtolower($_SESSION['imagesercurity']) == strtolower(clean_input('sercurity'))) {

        $a = array();

        $a['name'] = clean_input('yourname');
        $a['email'] =  clean_input('email');
        $a['phone'] =  clean_input('phone');
        $a['address'] =  clean_input('address');
        $a['other_request'] =  clean_input('other_request');

		
        $a['masothue'] =  clean_input('masothue');
        $a['tencongty'] =  clean_input('tencongty') ;
        $a['diachicongty'] = clean_input('diachicongty');
