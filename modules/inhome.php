...
function inHome()
{
  ...
  while ($rs_cat = $DBi->fetch_array($db_cat)) {
    ...
    if ($rs_cat['image'])
      $tpl1->assign("catimage", '<img src="' . $cache_image_path . cropimage(600, 600, $dir_path . '/' . $rs_cat['image']) . '" alt="' . $rs_cat['name'] . '" width="100%" >');

    if ($rs_cat['imageadv'])
      $tpl1->assign("catimageadv", '<img src="' . $rs_cat['imageadv'] . '" width="100%" >');


    $sql = "SELECT * FROM info WHERE active=1 AND id_category = $rs_cat[id_category] ORDER BY thu_tu ASC, name ASC";
    $db = $DBi->query($sql);
    while ($rs = $DBi->fetch_array($db)) {
      ...
      if ($rs['image'])
        $tpl1->assign('image_url', $rs['image']);
      // $tpl1->assign("image", '<img src="' . $cache_image_path . cropimage(100, 100, $dir_path . '/' . $rs['image']) . '" alt="' . $rs['name'] . '" width="100%" >');
