...
$tpl = new TemplatePower("skin/news.tpl");
$tpl->prepare();
$id = intval($_GET['id']);
$idc = intval($_GET['idc']);
$pid = intval($_GET['pid']);
...
class News extends cat_tree {
    ...
    private $page = "Trang Tin tức";
    private $item = "tin bài";
    private $table = "news";
    private $id_item = "id_news";
    private $par_page = "news";
    ...
    private function showAddNew() {
        ...
        $tpl->assign("listTacgia", $this->listTacgia());
    ...
    private function showUpdate() {
        ...
        if ($rs = $DBi->fetch_array($db)) {
            ...
            // list category
            $info['parentid1'] = intval( $rs['id_category'] ) ;
            $info['parentid'] .= '<select name="parentid" style="WIDTH: 300px" >';
            $info['parentid'] .= '<option value="0">Root</option>';
            if ($tree)
                foreach ($tree as $k => $v) {
            ...
            $tpl->assign("listTacgia", $this->listTacgia($rs['id_tacgia']));
    ...
    private function showList() {
        ...
        while ($rs = $DBi->fetch_array($db['db'])) {
            ...
            if ($rs['hot'] == 1)
                $tpl->assign("hot", "<div class='hot'></div>");
    ...
    private function getData($update = 0, $id = 0) {
        global $my, $_FILES, $CONFIG, $clsUrl, $lang;
        $id = intval($id);
        $data = array();
        ...
        $data['tags'] = compile_post('tags');
        $data['header_tag'] = compile_post('header_tag');


        $data['id_tacgia'] = intval(compile_post('id_tacgia'));

        $data['active'] = intval(compile_post('active'));
        $data['form_name'] = intval(compile_post('form_name'));
        ...
        $data['linkto'] = compile_post('linkto');
    ...
    function listTacgia($id = 0) {
        global $DBi, $tpl;
        $sql = "SELECT * FROM tacgia ORDER BY thu_tu DESC";
        $db = $DBi->query($sql);
        $str = '<select name="id_tacgia" ><option value="0">Chọn tác giả</option>';
        while ($rs = $DBi->fetch_array($db)) {
            if ($rs['id_tacgia'] == $id)
                $str .= '<option value="' . $rs['id_tacgia'] . '" selected>' . $rs['name'] . '</option>';
            else
                $str .= '<option value="' . $rs['id_tacgia'] . '" >' . $rs['name'] . '</option>';
        }
        $str .= '</select>';
        return $str;
    }
